...
品牌色：活力橙 (#C2410C) + 语义绿 (#059669)，字体 Barlow / Barlow Condensed。

## 近期 UI/UX 改进

- **品牌升级**：主色从翡翠绿改为活力橙，聚焦环与按钮统一暖色调；保留翡翠绿用于达标/完成等正向语义
- **字体个性**：正文 Barlow，标题 Barlow Condensed，运动感更强
- **微交互**：按钮按压反馈、卡片悬浮抬升、数字弹入、淡入上滑动画
- **无障碍**：全局按钮 44px、聚焦环可见、颜色对比度 5.02:1（通过 WCAG AA）
- **桌面日志页**：双栏布局，左侧常用录入+训练状态，右侧概况+补空工具
- **移动端日志页**：关键条目一屏完成；备注框始终可见不再隐藏；训练状态压缩为一行
- **移动端训练页**：底栏常驻，休息计时器置顶突出显示，三按钮直达核心操作
- **同步消息**：全部简化 ~50%，减少视觉干扰
...
- 顶部“导出”默认以当前选中日期为结束日导出近 30 天每日记录和训练记录，也可切换当天、本周、近 7 天、本月、全部或自定义日期。
