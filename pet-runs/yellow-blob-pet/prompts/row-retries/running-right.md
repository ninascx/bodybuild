Create Codex pet row `running-right` for `yellow-blob`: exactly 8 full-body frames in one horizontal strip on flat pure blue #0000FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Preserve the reference character: yellow rounded bean-like body, soft 3D toy material, tiny arms, short legs, huge glossy black eyes with pale yellow rims, small calm smile, no clothing, no props.. Preserve silhouette, face, palette, material, proportions, markings, and props.
